...
CREATE TABLE product_categories (
  ...
  description TEXT,
  field_schema JSONB DEFAULT '[]'::jsonb, -- 카테고리별 상품 상세 항목 스키마 (MetaField[])
...
-- 카테고리: 전체 공개 조회 + 어드민 전체 CRUD
CREATE POLICY "product_categories_select_public" ON product_categories FOR SELECT USING (true);
CREATE POLICY "product_categories_insert_admin" ON product_categories FOR INSERT WITH CHECK (public.is_admin());
CREATE POLICY "product_categories_update_admin" ON product_categories FOR UPDATE USING (public.is_admin());
CREATE POLICY "product_categories_delete_admin" ON product_categories FOR DELETE USING (public.is_admin());
...
COMMENT ON COLUMN product_categories.field_schema IS '카테고리별 상품 상세 항목 스키마. MetaField[] 형태: [{key,label,type,placeholder?,options?,span?}]. 관리자 화면(카테고리 등록/수정)에서 편집';
...
-- =============================================
-- 마이그레이션: 카테고리별 상세 항목 스키마(field_schema) 도입
-- 기존에 src/config/productMetaSchemas.ts에 하드코딩되어 있던
-- 도서/잡화 상세 항목을 DB 데이터로 이관하고, '이외' 카테고리를 추가
-- 2026-07-08
-- =============================================
...
DROP POLICY IF EXISTS "product_categories_insert_admin" ON product_categories;
DROP POLICY IF EXISTS "product_categories_update_admin" ON product_categories;
DROP POLICY IF EXISTS "product_categories_delete_admin" ON product_categories;
CREATE POLICY "product_categories_insert_admin" ON product_categories FOR INSERT WITH CHECK (public.is_admin());
CREATE POLICY "product_categories_update_admin" ON product_categories FOR UPDATE USING (public.is_admin());
CREATE POLICY "product_categories_delete_admin" ON product_categories FOR DELETE USING (public.is_admin());

-- 기존 하드코딩 스키마(productMetaSchemas.ts)를 데이터로 이관
UPDATE product_categories
SET field_schema = '[
  {"key":"shipping_fee",        "label":"배송비 (원)",  "type":"number",   "placeholder":"3000",                     "span":"half"},
  ...
  {"key":"book_type",           "label":"도서 형태",    "type":"select",   "options":["종이책","전자책","오디오북"], "span":"half"},
  {"key":"print_color",         "label":"인쇄 컬러",    "type":"select",   "options":["흑백","컬러","2도"],          "span":"half"},
  {"key":"age_limit",           "label":"연령 제한",    "type":"select",   "options":["전연령","12세 이상","15세 이상","18세 이상"], "span":"half"},
...
WHERE slug = 'books' AND field_schema = '[]'::jsonb;
...
WHERE slug = 'goods' AND field_schema = '[]'::jsonb;

-- '이외' 카테고리 신규 추가 (상세설명/비고는 공통 필드라 별도 항목 없이 빈 스키마)
INSERT INTO product_categories (name, slug, description, field_schema)
VALUES ('이외', 'etc', '도서/잡화 외 상품', '[]'::jsonb)
ON CONFLICT (slug) DO NOTHING;

-- =============================================
-- 마이그레이션: 상품 리뷰/Q&A 작성 기능
-- 리뷰는 주문(order_item) 단위로 작성 — 같은 상품을 재구매하면
-- 구매 건마다 별도로 리뷰를 남길 수 있다 (쿠팡 등과 동일한 모델).
-- 2026-07-08
-- =============================================
ALTER TABLE product_reviews
  ADD COLUMN IF NOT EXISTS order_item_id INTEGER REFERENCES order_items(id) ON DELETE SET NULL;

COMMENT ON COLUMN product_reviews.order_item_id IS '리뷰가 귀속된 구매 건(order_items.id). 주문 삭제 시에도 리뷰는 보존되도록 SET NULL';
...
-- 관리자 정책: 소유자(auth.uid() = user_id) FOR ALL 정책만 있고 관리자용 정책이 없어
-- 숨김 리뷰/비공개 문의를 어드민이 조회·수정·삭제할 수 없었다.
...
DROP POLICY IF EXISTS "product_reviews_delete_admin" ON product_reviews;
...
DROP POLICY IF EXISTS "product_qna_select_admin" ON product_qna;
DROP POLICY IF EXISTS "product_qna_update_admin" ON product_qna;
DROP POLICY IF EXISTS "product_qna_delete_admin" ON product_qna;
