-- =============================================
-- 강의(Academy) 핵심 테이블
-- 사전 조건: common.sql (set_updated_at, is_admin), order.sql (orders)
-- =============================================
...
-- =============================================
-- 수강 등록 테이블
-- INSERT는 /api/enroll (service role) 에서만 처리
-- =============================================
...
CREATE POLICY "lectures_read_published"
  ON lectures FOR SELECT USING (is_published = true);

CREATE POLICY "lecture_board_posts_read_published"
  ON lecture_board_posts FOR SELECT USING (is_published = true);

-- 수강 등록: 본인 조회
CREATE POLICY "lecture_enrollments_read_own"
  ON lecture_enrollments FOR SELECT USING (auth.uid() = user_id);

-- 시청 진도: 본인 조회/삽입/수정
CREATE POLICY "lecture_watch_progress_read_own"
  ON lecture_watch_progress FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "lecture_watch_progress_insert_own"
  ON lecture_watch_progress FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "lecture_watch_progress_update_own"
  ON lecture_watch_progress FOR UPDATE USING (auth.uid() = user_id);

-- 북마크: 본인 조회/추가/삭제
CREATE POLICY "lecture_bookmarks_read_own"   ON lecture_bookmarks FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "lecture_bookmarks_insert_own" ON lecture_bookmarks FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "lecture_bookmarks_delete_own" ON lecture_bookmarks FOR DELETE USING (auth.uid() = user_id);

-- 어드민 CRUD
CREATE POLICY "lectures_read_admin"   ON lectures FOR SELECT USING (public.is_admin());
CREATE POLICY "lectures_insert_admin" ON lectures FOR INSERT WITH CHECK (public.is_admin());
CREATE POLICY "lectures_update_admin" ON lectures FOR UPDATE USING (public.is_admin());
CREATE POLICY "lectures_delete_admin" ON lectures FOR DELETE USING (public.is_admin());

CREATE POLICY "lecture_chapters_insert_admin" ON lecture_chapters FOR INSERT WITH CHECK (public.is_admin());
CREATE POLICY "lecture_chapters_update_admin" ON lecture_chapters FOR UPDATE USING (public.is_admin());
CREATE POLICY "lecture_chapters_delete_admin" ON lecture_chapters FOR DELETE USING (public.is_admin());

CREATE POLICY "lecture_sessions_insert_admin" ON lecture_sessions FOR INSERT WITH CHECK (public.is_admin());
CREATE POLICY "lecture_sessions_update_admin" ON lecture_sessions FOR UPDATE USING (public.is_admin());
CREATE POLICY "lecture_sessions_delete_admin" ON lecture_sessions FOR DELETE USING (public.is_admin());

CREATE POLICY "lecture_board_posts_select_admin" ON lecture_board_posts FOR SELECT USING (public.is_admin());
CREATE POLICY "lecture_board_posts_insert_admin" ON lecture_board_posts FOR INSERT WITH CHECK (public.is_admin());
CREATE POLICY "lecture_board_posts_update_admin" ON lecture_board_posts FOR UPDATE USING (public.is_admin());
CREATE POLICY "lecture_board_posts_delete_admin" ON lecture_board_posts FOR DELETE USING (public.is_admin());
